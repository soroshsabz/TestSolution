...
namespace BSN.Resa.Locale
{
    public static class CultureHelper
	{
		...
		{
            return System.Globalization.CultureInfo.CurrentUICulture.Name == "fa-IR";
		}
		...
		public static string GetImplementedCulture(string name)
		{
			...
			if (ValidCultures.Count(c => c.Equals(name, StringComparison.CurrentCultureIgnoreCase)) == 0)
				return GetDefaultCulture(); // return Default culture if it is invalid
			// if it is implemented, accept it
			if (Cultures.Count(c => c.Equals(name, StringComparison.CurrentCultureIgnoreCase)) > 0)
		...
		{
			return GetNeutralCulture(System.Globalization.CultureInfo.CurrentUICulture.Name);
		}
	    ...
	    public static SupportedLanguages GetCurrentLanguage(CultureInfo cultureInfo)
	    {
	        SupportedLanguages currentLanguages;

	        if (string.IsNullOrWhiteSpace(cultureInfo?.Name))
	            return SupportedLanguages.Farsi;

	        var cultureShortName = CultureHelper.GetNeutralCulture(cultureInfo.Name);

	        switch (cultureShortName.ToLower())
	        {
	            case "fa":
	            {
	                currentLanguages = SupportedLanguages.Farsi;
	                break;
	            }
	            case "en":
	            {
	                currentLanguages = SupportedLanguages.English;
	                break;
	            }
	            default:
	            {
